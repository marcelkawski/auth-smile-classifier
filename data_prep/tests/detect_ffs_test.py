...
def test_consecutive_frames_in_ffs_data_files():
    ...
    for dfn in data_files_names:
        path = os.path.abspath(os.path.join(os.sep, FACES_FEATURES_DATA_DIR, dfn))
...
def test_created_data_names():
    ffs_dirs_names = sorted(get_all_subdirs(FACES_FEATURES_DIR))
    ffs_data_filenames = sorted([os.path.splitext(name)[0] for name in get_all_filenames(FACES_FEATURES_DATA_DIR)])
    assert ffs_data_filenames == ffs_dirs_names


def test_created_data_num():
    ffs_imgs_dirs_names = get_all_subdirs(FACES_FEATURES_DIR)
    for dirname in ffs_imgs_dirs_names:
        ffs_imgs_path = os.path.abspath(os.path.join(os.sep, FACES_FEATURES_DIR, dirname))
        ffs_data_file_path = os.path.abspath(os.path.join(os.sep, FACES_FEATURES_DATA_DIR, dirname + '.csv'))
